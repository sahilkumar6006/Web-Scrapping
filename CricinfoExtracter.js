...
// node CricinfoExtracter.js --excel=Worldcup.csv --dataDir=data --source=https://www.espncricinfo.com/series/icc-cricket-world-cup-2019-1144415/match-results  
...
responseKaPromise.then(function (response) {
    ...
    let matchScoreDivs = document.querySelectorAll("div.ds-grow.ds-px-4");
    let matches = [];
    ...
    for (let i = 0; i < matchScoreDivs.length; i++) {
        ...
        let teamParas = matchScoreDivs[i].querySelectorAll("p.ds-text-tight-m.ds-font-bold.ds-capitalize");
        match.t1 = teamParas[0].textContent;
        match.t2 = teamParas[1].textContent;

        let scoreSpans = matchScoreDivs[i].querySelectorAll("div.ds-text-compact-s > strong");
        ...
        let resultSpan = matchScoreDivs[i].querySelector("p.ds-text-tight-s");
